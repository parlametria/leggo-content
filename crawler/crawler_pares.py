...
def obtem_texto_original_e_html_tramitacao(link_originaria, anos, numero_PL_MPV, link_para_PL_MPV):
	time.sleep(0.1)
...
def salva_dados(anos, dia_e_mes, numero_PL_MPV, fonte_originaria, texto_publicado, requisicao, link_para_PL_MPV, link_originaria, link_publicado):
	with open('doc_' + anos[indice_anos] + "_" + dia_e_mes + str(numero_PL_MPV) + '.txt', 'w') as arquivo:
		arquivo.write(fonte_originaria)

	with open('doc_' + anos[indice_anos] + "_" + dia_e_mes  + str(numero_PL_MPV) + '.txt', 'w') as arquivo:
		arquivo.write(texto_publicado)

	with open('doc_' + anos[indice_anos] + "_" + dia_e_mes + str(numero_PL_MPV) + '.pdf', 'wb') as arquivo:
		arquivo.write(requisicao.content)

	with open('readme.txt', 'a') as arquivo:
	...
	while(1): #acaba quando não houverem mais páginas
		for link_para_PL_MPV in soup_principal.find_all('span', class_ = 'titulo'): #todos spans com classe titulo em cada página inicial contém um link
			...
			try: #para o caso de alguma das tags e classes mencionadas no código não existir na coleta em questão ou afins
				...
				requisicao, fonte_originaria = obtem_texto_original_e_html_tramitacao(link_originaria, anos, numero_PL_MPV, link_para_PL_MPV)
				
				############################ Salvando os arquivos (colocados no final para que o bloco try-except funcione corretamente)
				salva_dados(anos, dia_e_mes, numero_PL_MPV, fonte_originaria, texto_publicado, requisicao, link_para_PL_MPV, link_originaria, link_publicado)
